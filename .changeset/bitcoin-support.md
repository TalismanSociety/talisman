...
- `@talismn/bitcoin`: new package — Esplora REST client (failover, fee normalization), HD gap-limit scanner (dual tree, block-aware), PSBT build/sign/finalize/verify (`@scure/btc-signer`, coin selection, taproot, BIP125 RBF signaling, anti-fee-sniping locktime), BIP125 replacement builder (speed-up / cancel with context reconstruction from broadcast hex), BIP21 payment URI parse/encode, BIP322 "simple" message sign/verify, and `isBitcoinAddressValidForNetwork` (network-aware address validation)
- `@talismn/crypto`: Bitcoin derivation (BIP84 P2WPKH + BIP86 P2TR), xpub detect/normalize, SLIP-132 prefix detection and display encoding (zpub/vpub), P2WPKH/P2TR address encoders, WIF parse/encode; `bip32-xpub` address encoding; `bitcoin-ecdsa` curve now maps to bech32 and the dead `bitcoin-ed25519` curve is removed
