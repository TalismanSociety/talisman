...
- `@talismn/chaindata-provider`: `BtcNetwork` / `BtcNativeToken` schemas, network/token union members, and bitcoin platform guards; block explorer urls route xpub queries to xpub-capable explorers (blockbook `/xpub/` path) and skip explorers without xpub support
