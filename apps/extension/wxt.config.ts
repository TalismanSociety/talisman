...
// Load supported languages from languages.json if not set in environment.
// The languages.json file is generated by scripts/download-translations.ts
// which fetches available languages from SimpleLocalize.
const getSupportedLanguages = (): string => {
  if (process.env.SUPPORTED_LANGUAGES) return process.env.SUPPORTED_LANGUAGES

  const languagesPath = resolve(__dirname, "public/locales/languages.json")
  if (existsSync(languagesPath)) {
    try {
      ...
      // Fall through to empty string if file is invalid
    }
  }
  return ""
}
