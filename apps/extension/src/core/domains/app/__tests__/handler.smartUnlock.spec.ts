...
describe("App handler smart unlock", () => {
  ...
  beforeEach(async () => {
    ...
    await extensionStores.smartUnlock.unenroll()
  ...
  test("cannot enroll while logged out", async () => {
    ...
    await expect(
      messageSender("pri(app.smartUnlock.enroll)", enrollRequest(randomPrfOutput()))
    ).rejects.toThrow("Must be logged in to enable smart unlock")

    expect(await messageSender("pri(app.smartUnlock.getCredentialInfo)", null)).toBeNull()
  })
  ...
  test("cannot enroll without an auth secret", async () => {
    ...
    await expect(
      messageSender("pri(app.smartUnlock.enroll)", enrollRequest(randomPrfOutput()))
    ).rejects.toThrow("Please log in again before enabling smart unlock")
  })

  test("unlocks the wallet with the enrolled PRF output", async () => {
    const prfOutput = randomPrfOutput()
    expect(await messageSender("pri(app.smartUnlock.enroll)", enrollRequest(prfOutput))).toBe(true)

    await messageSender("pri(app.lock)", null)
    expect(extensionStores.password.isLoggedIn.value).toBe("FALSE")

    expect(await messageSender("pri(app.smartUnlock.authenticate)", { prfOutput })).toBe("success")
  ...
  test("only exposes the public part of the enrollment", async () => {
    ...
    await messageSender("pri(app.smartUnlock.enroll)", request)

    expect(await messageSender("pri(app.smartUnlock.getCredentialInfo)", null)).toEqual({
  ...
  test("stays locked and clears the enrollment on a wrong PRF output", async () => {
    await messageSender("pri(app.smartUnlock.enroll)", enrollRequest(randomPrfOutput()))
    await messageSender("pri(app.lock)", null)

    expect(
      await messageSender("pri(app.smartUnlock.authenticate)", { prfOutput: randomPrfOutput() })
    ).toBe("unenrolled")
  ...
  test("keeps the session and the enrollment when called while already unlocked", async () => {
    ...
    expect(extensionStores.password.isLoggedIn.value).toBe("TRUE")
    expect(await messageSender("pri(app.smartUnlock.getCredentialInfo)", null)).not.toBeNull()
  })
  ...
  test("keeps the enrollment when the auth secret is missing", async () => {
    const prfOutput = randomPrfOutput()
    await messageSender("pri(app.smartUnlock.enroll)", enrollRequest(prfOutput))
    await messageSender("pri(app.lock)", null)

    // the next password login sets the auth secret back up, so this enrollment isn't dead
    await extensionStores.password.set({ secret: undefined, check: undefined })

    expect(await messageSender("pri(app.smartUnlock.authenticate)", { prfOutput })).toBe("failed")
    expect(await messageSender("pri(app.smartUnlock.getCredentialInfo)", null)).not.toBeNull()
  })
  ...
  test("unenroll clears the enrollment", async () => {
    ...
    expect(await messageSender("pri(app.smartUnlock.unenroll)", null)).toBe(true)
    expect(await messageSender("pri(app.smartUnlock.getCredentialInfo)", null)).toBeNull()
  })
  ...
  test("cannot unenroll while logged out", async () => {
    ...
    await expect(messageSender("pri(app.smartUnlock.unenroll)", null)).rejects.toThrow(
