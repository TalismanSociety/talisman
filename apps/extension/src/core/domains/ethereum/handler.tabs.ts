...
export class EthTabsHandler extends TabsHandler {
  ...
  private async getPublicClient(url: string, authorisedAddress?: string): Promise<PublicClient> {
    ...
    const ethereumNetwork = await chaindataProvider.getEvmNetworkById(site.ethChainId.toString())
    if (!ethereumNetwork)
  ...
  private addEthereumChain = async (
    ...
    const existing = await chaindataProvider.getEvmNetworkById(chainId.toString())
    ...
    const ethereumNetwork = await chaindataProvider.getEvmNetworkById(chainId.toString())
    if (ethereumNetwork) {
  ...
  private switchEthereumChain = async (
    ...
    const ethereumNetwork = await chaindataProvider.getEvmNetworkById(ethChainId.toString())
    const activeNetworks = await activeEvmNetworksStore.get()
