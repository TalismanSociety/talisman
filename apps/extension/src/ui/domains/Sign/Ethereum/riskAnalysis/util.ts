...
import { TenderlyChange } from "./types"
...
type OperationSide = "Send" | "Receive"

export const getOperationSide = (change: TenderlyChange, signer: string): OperationSide | null => {
  if (change.kind === "asset")
    if (isAddressEqual(change.to, signer)) return "Receive"
    else if (change.from && isAddressEqual(change.from, signer)) return "Send"

  if (change.kind === "exposure") return "Send"

  return null
}

export const isPositiveStateChange = (change: TenderlyChange, signer: string) => {
  // TODO null if not a transfer ?

  if (change.kind === "asset") {
    return isAddressEqual(change.to, signer)
  }

  if (change.kind === "exposure") {
    return !isAddressEqual(change.spender, signer)
  }
