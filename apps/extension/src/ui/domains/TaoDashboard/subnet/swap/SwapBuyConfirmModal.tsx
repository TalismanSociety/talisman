...
const AlphaPriceValue = () => {
  const { swapPrice, tokenIdIn } = useSwapBuy()
...
const FeeEstimateValue = () => {
  const { feeEstimate, tokenIn, isLoadingFeeEstimate, errorFeeEstimate } = useSwapBuy()
...
const TalismanFeeValue = () => {
  const isSeekTaoDiscountEnabled = useFeatureFlag("SEEK_TAO_DISCOUNT")
  const { talismanFee, tokenIn, isLoading, errorFeeEstimate } = useSwapBuy()
