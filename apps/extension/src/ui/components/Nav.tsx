...
export const Nav: FC<DetailedHTMLProps<HTMLAttributes<HTMLElement>, HTMLElement>> = ({
  className,
  ...props
}) => <nav className={cn("flex flex-col items-start justify-start", className)} {...props} />
...
export const NavItem: FC<NavItemProps> = ({
  ...
  const iconContainer = icon && (
    <div className={cn("flex w-20 shrink-0 justify-center text-lg", iconContainerClassName)}>
      {icon}
    </div>
  )
  const content = (
