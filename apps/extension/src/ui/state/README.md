...
**Pattern:** Each file subscribes to background data via `api` (from `ui/api/`) or a shared core store (e.g. `remoteConfigStore`), wraps it in an RxJS `Observable`, and uses `@react-rxjs/core`'s `bind()` to produce React hooks.
...
**What doesn't belong here:** State used by a single domain lives in that domain, as a `provideContext` provider or hooks (e.g., `ui/domains/Swap/SwapProvider.tsx`).
