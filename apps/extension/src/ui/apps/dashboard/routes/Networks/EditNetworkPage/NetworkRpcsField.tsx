...
//       const info = await getSubstrateRpcInfo(rpcUrl)
//       if (!info) throw new Error("Failed to info from RPC")

//       if (onChainId && info.genesisHash !== onChainId)
//         throw new Error("RPC doesn't match chain's genesis hash")
...
//       console.log("Chain ID fetched from RPC:", hexChainId)
//       const chainId = String(hexToNumber(hexChainId as `0x${string}`)) // validate chain ID
//       console.log("numeric Chain ID:", chainId)

//       if (onChainId && chainId !== onChainId) throw new Error("RPC doesn't match chain's chain ID")
