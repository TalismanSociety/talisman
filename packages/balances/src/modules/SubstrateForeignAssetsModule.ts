...
export const subForeignAssetTokenId = (chainId: ChainId, assetId: string, tokenSymbol: string) =>
  `${chainId}-substrate-foreignassets-${assetId}-${tokenSymbol}`.toLowerCase().replace(/ /g, "-")
    ...
    async fetchSubstrateChainTokens(chainId, chainMeta, moduleConfig) {
      ...
      for (const tokenConfig of moduleConfig?.tokens ?? []) {
        try {
          ...
          const id = subForeignAssetTokenId(chainId, assetId, symbol)
          ...
          log.error(
            `Failed to build substrate-foreignassets token ${tokenConfig.assetId} (${tokenConfig.symbol}) on ${chainId}`,
            error
          )
          continue
        }
