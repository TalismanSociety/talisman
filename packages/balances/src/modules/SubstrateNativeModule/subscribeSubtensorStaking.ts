...
import { isAbortError, isEthereumAddress } from "@talismn/util"
...
export async function subscribeSubtensorStaking(
  ...
  try {
      ...
      const subtensorQueries = from(addresses).pipe(
        ...
        map((stakes) =>
          stakes.map(({ address, hotkey, stake, netuid, dynamicInfo }): SubNativeBalance => {
            ...
            const stakeByNetuid =
              Number(netuid) === SUBTENSOR_ROOT_NETUID ? stake : alphaStakedInTao
    ...
    if (!isAbortError(err)) log.error("Error subscribing to subtensor staking", { err })
    return () => {}
  }
}
