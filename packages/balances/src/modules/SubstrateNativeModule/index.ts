...
> = (hydrate) => {
  ...
  const subscribeChainBalances = async (
    ...
    const initialisingBalances = Object.entries(addressesByToken).reduce<Map<TokenId, Set<string>>>(
  ...
  const fetchBalances = async (addressesByToken: AddressesByToken<SubNativeToken>) => {
    ...
    async subscribeBalances({ addressesByToken, initialBalances }, callback) {
      ...
      const safeCallback: SubscriptionCallback<Balances | SubscriptionResultWithStatus> = (
        ...
        return error
          ...
          : callback(error, result as Balances | SubscriptionResultWithStatus)
      }

      const unsubsribeFns = Promise.all(
