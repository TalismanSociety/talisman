...
import { isAbortError, isEthereumAddress, isNotNil } from "@talismn/util"
...
export async function subscribeNompoolStaking(
  ...
  try {
    ...
    for (const [tokenId, addresses] of Object.entries(addressesByNomPoolToken)) {
      ...
      const subscribePoolStake = (
        poolIds: string[],
        callback: SubscriptionCallback<PoolStake[]>,
      ) => {
        if (poolIds.length === 0) callback(null, [])
    ...
    if (!isAbortError(err)) log.error("Error subscribing to nom pool staking", { err })
    return () => {}
  }
}
