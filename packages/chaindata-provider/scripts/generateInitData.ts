...
async function generateInitData() {
  ...
  fs.writeFileSync(
    path.resolve(__dirname, "../src/state/initChaindata.ts"),
    `export const networks = ${JSON.stringify(initData.networks, null, 2)}`,
  )
  ...
  execSync(`prettier --write '${path.resolve(__dirname, "../src/state/initChaindata.ts")}'`, {
    stdio: "inherit",
  })
}
