...
// [talisman] CustomSignedExtensions added
import {
...
import {
  EMPTY_SIGNED_EXTENSION,
  empty,
  signedExtension,
} from "./signed-extensions/utils"
  ...
  const fromPjsToInputRaw = (
    ...
    const signedExtensions = lookup.metadata.extrinsic.signedExtensions[0].map(
      ({ identifier, type, additionalSigned }): SignedExtension => {
        ...
        // [talisman] caller-provided encoders for signed extensions unknown to this library
        const custom = customSignedExtensions?.[identifier]
        if (custom)
          return custom(
            pjsPayload as SignerPayloadJSON & Record<string, unknown>,
            metadata,
          )
